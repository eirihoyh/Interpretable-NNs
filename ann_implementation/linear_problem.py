...
for ni in range(n_nets):
    ...
    pf.run_path_graph_weight_ann(net, save_path=f"ann_implementation/path_graphs/net{ni}_linear_relu", show=False)
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'ann_implementation/results/ann_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_lin_func_relu_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'ann_implementation/results/ann_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_lin_func_relu_median.txt',m_median,delimiter = ',')
