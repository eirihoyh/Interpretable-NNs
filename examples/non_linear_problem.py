...
for ni in range(n_nets):
    ...
    if save_res:
        torch.save(net, f"examples/network/net{ni}_non_linear")
    all_nets[ni] = net 
    # Results
    ...
    pf.run_path_graph(all_nets[ni], threshold=0.5, save_path=f"examples/path_graphs/prob/net{ni}_non_linear_relu", show=False)
    pf.run_path_graph_weight(net, save_path=f"examples/path_graphs/weight/net{ni}_non_linear_relu", show=False)
...
if save_res:
    # m = np.array(metrics_several_runs)
    np.savetxt(f'examples/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_non_lin_func_relu_full.txt',m,delimiter = ',')
    # m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'examples/results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_non_lin_func_relu_median.txt',m_median,delimiter = ',')
